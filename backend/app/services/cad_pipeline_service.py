...
# ---------------------------------------------------------------------------
# Centralised task_id validation
# ---------------------------------------------------------------------------
# CAD task ids are generated by the system as ``uuid.uuid4().hex[:8]`` (8
# lowercase hex chars).  They are used directly as sub-directory names under
# ``outputs/cad_tasks/`` and as lock / marker filename components.  Because
# these strings are joined onto filesystem paths, an externally supplied
# ``task_id`` containing ``../``, ``.`` segments or path separators would let a
# caller resolve or delete paths *outside* the task tree (a path traversal).
#
# Admin-token authentication proves *who* is calling, not that a given
# ``task_id`` names a real task — it does **not** stop path traversal.  So every
# public endpoint / service method that accepts a ``task_id`` must reject any
# value that is not an exact system-generated id before it is ever joined onto
# a path.  All task routes and service entry points funnel through
# :func:`validate_task_id` below (defence in depth).
...
class CADPipelineService:
    ...
    def build_download_package(self, task_ids: list[str]) -> tuple[Path, str]:
        # Reject any path-like / non-system task id before it is resolved.
